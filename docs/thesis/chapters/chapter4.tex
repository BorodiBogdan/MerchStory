...
Chapter~\ref{chap:architecture} treated the recommendation engine and the hybrid catalogue mode as services with a fixed contract. This chapter looks inside both. A shorter section at the end describes the print pipeline that supports them.

The same idea runs through all three. The gap between what an off-the-shelf generative model produces and what a small retailer is willing to publish is rarely closed by a bigger model. It is closed by the protocol that sits around the model. The recommendation engine grounds the language model in retrieved context and a multi-stage prompt chain. The hybrid catalogue pipeline restricts what the image model is allowed to render and lets a deterministic compositor enforce that restriction. The print pipeline tiles a small super-resolution model under fixed paper sizes.
...
Small retailers know their products and their customers, but they rarely have time to plan a daily marketing schedule. Out-of-the-box language models, asked to suggest a promotion for today, default to season-blind, vendor-shaped boilerplate: ``discover our premium selection'', ``don't miss out on our exclusive offer''. The output is grammatical but useless. The recommendation engine's job is to produce, every day, a small ranked set of fresh marketing themes that are grounded in (a) the day's external context, (b) a curated promotional playbook of patterns that have worked in retail, and (c) the shop's own profile, while not repeating what the shop has already seen in the recent past.

The engine's output is not a final asset, but it is more than just copy. Each suggestion is a structured record that carries a title, body copy, a suggested social-media caption, an asset \emph{category} (\emph{announcement} or \emph{promotion}) and an \texttt{imagePrompt} written specifically for the image-generation pipeline of Section~\ref{sec:hybrid-catalogue}. Tapping ``generate'' on an idea hands the retailer to the asset-creation screen with the prompt and category already filled in, so that the path from a daily idea to a published asset is a single further confirmation. The engine is therefore the entry point of the platform's full creation flow, not a side feature. Figure~\ref{fig:rec-handoff} sketches that flow.
...
\begin{tikzpicture}[
    ...
    card/.style={draw, rounded corners=2pt, minimum width=34mm, minimum height=20mm, align=center, fill=gray!8},
    screen/.style={draw, rounded corners=2pt, minimum width=30mm, minimum height=14mm, align=center, fill=yellow!12},
    pipe/.style={draw, rounded corners=2pt, minimum width=30mm, minimum height=14mm, align=center, fill=blue!8},
    asset/.style={draw, rounded corners=2pt, minimum width=26mm, minimum height=12mm, align=center, fill=gray!10},
    arr/.style={-{Latex[length=1.6mm]}, thick}
]

\node[card] (idea) at (0, 0) {%
    \textbf{Daily idea}\\[2pt]
    \scriptsize title, body, suggestedPost,\\
    \scriptsize category, imagePrompt
};
\node[font=\tiny\itshape] at (0, -1.7) {(recommendation engine)};

\node[screen] (screen) at (5.0, 0) {Asset-creation\\screen\\\scriptsize prompt and category\\\scriptsize pre-filled};

\node[pipe] (pipe) at (10.0, 0) {Generation\\pipeline\\\scriptsize (catalogue or\\\scriptsize announcement)};

\node[asset] (out) at (14.0, 0) {Final asset\\in gallery};

\draw[arr] (idea) -- node[above, font=\tiny]{\itshape tap ``generate''} (screen);
\draw[arr] (screen) -- node[above, font=\tiny]{\itshape confirm} (pipe);
\draw[arr] (pipe) -- (out);
...
\begin{tikzpicture}[
    every node/.style={font=\scriptsize},
    box/.style={draw, rounded corners=2pt, minimum width=26mm, minimum height=10mm, align=center, fill=gray!8},
    proc/.style={draw, rounded corners=2pt, minimum width=26mm, minimum height=8mm, align=center, fill=gray!12},
    train/.style={draw, rounded corners=2pt, minimum width=26mm, minimum height=8mm, align=center, fill=blue!8},
    rag/.style={draw, rounded corners=2pt, minimum width=30mm, minimum height=7mm, align=center, fill=yellow!15},
    store/.style={draw, cylinder, shape border rotate=90, aspect=0.25, minimum width=22mm, minimum height=12mm, align=center, fill=gray!5},
    person/.style={draw, circle, minimum size=8mm, fill=gray!5, align=center, font=\tiny},
    arr/.style={-{Latex[length=1.6mm]}, thick},
    fb/.style={-{Latex[length=1.6mm]}, dashed, thick, gray!75}
]

% --- Top: corpus
\node[box] (data) at (0, 5.4) {Curated retail\\marketing corpus};

% --- Left branch: indexing
\node[proc]  (split) at (-4.5, 3.6) {split, chunk, embed};
\node[store] (vec)   at (-4.5, 1.6) {pgvector\\(playbook +\\past ideas)};

\begin{pgfonlayer}{background}
\node[draw=gray!55, dashed, rounded corners=4pt, inner sep=2.8mm,
      fit=(split)(vec),
      label={[font=\scriptsize\itshape, gray!75]above:Indexing}] (idxbox) {};
\end{pgfonlayer}

% --- Right branch: fine-tuning
\node[train] (td)    at ( 4.5, 3.9) {training pairs};
\node[train] (ft)    at ( 4.5, 2.6) {LoRA fine-tune\\(7B base)};
\node[train] (ftllm) at ( 4.5, 1.2) {fine-tuned\\writer LLM};

\begin{pgfonlayer}{background}
\node[draw=blue!50, dashed, rounded corners=4pt, inner sep=2.8mm,
      fit=(td)(ft)(ftllm),
      label={[font=\scriptsize\itshape, blue!65]above:Fine-tuning}] (ftbox) {};
\end{pgfonlayer}

% --- Bottom: inference flow
\node[person] (retailer) at (-8.4, -1.6) {retailer};
\node[box]    (app)      at (-5.4, -1.6) {app};

\node[rag] (sig) at (-0.4, -0.5) {context signals};
\node[rag] (req) at (-0.4, -1.4) {shop profile};
\node[rag] (ctx) at (-0.4, -2.3) {retrieved playbook\\+ past ideas};

\begin{pgfonlayer}{background}
\node[draw=yellow!55!black, dashed, rounded corners=4pt, inner sep=2.5mm,
      fit=(sig)(req)(ctx),
      label={[font=\scriptsize\itshape]above:RAG prompt}] (ragbox) {};
\end{pgfonlayer}

\node[box] (out) at ( 8.0, -1.6) {ideas};

% --- Arrows: corpus splits into both branches
\draw[arr] (data.south) -- (split.north);
\draw[arr] (data.south) -- (td.north);
...
\draw[arr] (vec.south) to[out=-90, in=180] (ctx.west);
\draw[arr] (ragbox.east) to[out=0, in=-90] (ftllm.south);
\draw[arr] (ftllm.south east) to[out=-30, in=90] (out.north);

% Feedback loop
\draw[fb] (out.south) to[out=-90, in=-90, looseness=0.55]
  node[below, font=\tiny\itshape, gray!75, sloped]{retailer feedback}
  (retailer.south);

\end{tikzpicture}%
}
\caption{Architecture of the recommendation engine: indexing and fine-tuning offline, RAG inference online, with retailer feedback closing the loop.}
...
\subsection{External context signals}
...
\paragraph{Weather.} A short-range forecast detects heatwaves, cold snaps and heavy rain, and turns each event into a signal tagged \emph{high}, \emph{medium} or \emph{low} with a one-line summary the prompt can read directly.

\paragraph{Holidays.} Upcoming public holidays in the shop's country are emitted over a fourteen-day lookahead, with severity scaling by proximity: \emph{high} at most three days away, \emph{medium} at most seven, \emph{low} further out.

\paragraph{News.} Recent news items in the shop's country are emitted at \emph{low} severity as ambient context, useful for seeding an angle but never for overriding the day's plan.
...
\hline
Weather  & \emph{High} for heatwave or cold snap, \emph{medium} for sustained warm or cool spells, \emph{low} for heavy-rain probability $\geq 70\,\%$. \\
\hline
Holiday  & \emph{High} at $\leq 3$ days, \emph{medium} at $\leq 7$, \emph{low} further out. \\
\hline
News     & Always \emph{low}, ambient context only. \\
\hline
\end{tabularx}
...
The playbook is partitioned by \emph{business domain}. The platform ships with a dedicated playbook for each of the four named domains it supports. \emph{Market} covers groceries, household goods, snacks and drinks. \emph{Food} covers caf\'es, bakeries and prepared meals. \emph{Retail} covers general-goods shops. \emph{Fashion} covers clothing and accessories. A fifth \emph{Other} playbook of domain-agnostic patterns takes over for shops whose domain falls outside the four. Adding a new named domain is purely an authoring exercise. A fresh set of entries is ingested under the same schema, no code change is required, and the engine picks the right playbook by matching the shop's recorded domain.
...
A single language-model call, at temperature $0.4$, is given the shop profile, the top context signals, the three retrieved playbook entries and a brief instruction to produce $N$ \emph{distinct angles}. Its output is a JSON document of the form
\[
...
\hline
Translator  & one per idea, parallel  & 0.4 & translated counterpart of the writer output \\
\hline
\end{tabularx}
...
Several language models were trialled against the same fixed batch of $(\text{shop}, \text{day}, \text{signal})$ inputs and compared on three axes: tonal fit, specificity to the \texttt{triggerSignal}, and JSON-schema adherence. The candidates were \emph{Gemma 3B}, \emph{DeepSeek}, a LoRA-fine-tuned seven-billion-parameter writer, and the enterprise APIs \emph{Claude} and \emph{Gemini}. The enterprise options produced the best output overall, with the cleanest structured responses and the fewest generic CTAs. The fine-tuned writer came close on tonal fit and specificity at a fraction of the hosting cost, while \emph{Gemma 3B} and \emph{DeepSeek} drifted back to boilerplate phrasing on weaker signals. Table~\ref{tab:rec-model-comparison} summarises the comparison. That gap is the immediate motivation for the writer-stage fine-tuning described next.
...
\begin{tikzpicture}[
    every node/.style={font=\scriptsize},
    svc/.style={draw, rounded corners=2pt, minimum width=32mm, minimum height=10mm, align=center, fill=gray!10},
    iface/.style={draw, rounded corners=2pt, minimum width=44mm, minimum height=14mm, align=center, fill=yellow!15},
    prov/.style={draw, rounded corners=2pt, minimum width=28mm, minimum height=10mm, align=center, fill=blue!8},
    arr/.style={-{Latex[length=1.6mm]}, thick}
]

\node[svc] (ann) at (0, 1.6)  {Announcement\\service};
\node[svc] (wal) at (0, 0)    {Wallpaper\\service};
\node[svc] (cat) at (0, -1.6) {Catalogue\\service};

\node[iface] (iface) at (5.6, 0) {\texttt{IImageProvider}\\\scriptsize\texttt{GenerateAsync(}\\\scriptsize\texttt{prompt, inlineImages)}};

\node[prov, minimum width=36mm, minimum height=14mm] (prov) at (10.8, 0) {Image generation providers\\\scriptsize (Gemini, OpenAI, \dots)};

\draw[arr] (ann.east) -- (iface.west);
\draw[arr] (wal.east) -- (iface.west);
\draw[arr] (cat.east) -- (iface.west);

\draw[arr] (iface.east) -- (prov.west);
...
\begin{tikzpicture}[
    every node/.style={font=\scriptsize},
    ref/.style   ={draw=black!55,        line width=0.5pt, rounded corners=3pt, minimum width=30mm, minimum height=12mm, align=center, fill=gray!8},
    model/.style ={draw=blue!55!black,   line width=0.5pt, rounded corners=3pt, minimum width=30mm, minimum height=12mm, align=center, fill=blue!6},
    cmp/.style   ={draw=orange!65!black, line width=0.5pt, rounded corners=3pt, minimum width=30mm, minimum height=12mm, align=center, fill=orange!10},
    bad/.style   ={draw=red!55!black,    line width=0.6pt, rounded corners=3pt, minimum width=30mm, minimum height=12mm, align=center, fill=red!8},
    good/.style  ={draw=green!50!black,  line width=0.6pt, rounded corners=3pt, minimum width=30mm, minimum height=12mm, align=center, fill=green!10},
    arr/.style   ={-{Latex[length=2mm,width=1.6mm]}, thick, draw=black!70},
    arrlbl/.style={font=\tiny\itshape, fill=white, inner sep=1pt},
    rowlbl/.style={font=\small\bfseries\itshape, anchor=east, text=black!70}
]

% --- (a) pure generative ---
\node[ref]   (refA) at (0,    2.6) {Reference\\photographs};
\node[model] (mdlA) at (4.2,  2.6) {Multimodal\\model};
\node[bad]   (outA) at (8.4,  2.6) {Final image\\\textit{products distorted}};
\draw[arr] (refA) -- (mdlA);
\draw[arr] (mdlA) -- (outA);

% --- (b) hybrid ---
\node[ref]   (refB) at (0,    0) {Reference\\photographs};
\node[model] (mdlB) at (4.2,  0) {Multimodal model\\\scriptsize(outline mode)};
\node[cmp]   (cmpB) at (8.4,  0) {Deterministic\\compositor};
\node[good]  (outB) at (12.6, 0) {Final image\\\textit{products faithful}};
\draw[arr] (refB) -- (mdlB);
\draw[arr] (mdlB) -- node[arrlbl, above=1pt]{outlined scene} (cmpB);
\draw[arr] (cmpB) -- (outB);

% --- Real reference photographs also feed the compositor ---
\draw[arr, dashed, draw=black!55] (refB.south)
    to[out=-55, in=-125, looseness=0.9]
    node[arrlbl, below=2pt]{real product photos}
    (cmpB.south);

% --- Row badges ---
\node[rowlbl] at (-1.7, 2.6) {(a) pure};
\node[rowlbl] at (-1.7, 0)   {(b) hybrid};
...
\begin{tikzpicture}[
    every node/.style={font=\scriptsize},
    inp/.style    ={draw=black!50,        line width=0.4pt, rounded corners=2pt,   minimum width=28mm, minimum height=8mm,  align=center, fill=gray!6,   font=\scriptsize\itshape},
    proc/.style   ={draw=gray!65,         line width=0.5pt, rounded corners=2.5pt, minimum width=32mm, minimum height=11mm, align=center, fill=gray!12},
    aicall/.style ={draw=blue!60!black,   line width=0.7pt, rounded corners=2.5pt, minimum width=34mm, minimum height=12mm, align=center, fill=blue!8},
    art/.style    ={draw=yellow!60!black, line width=0.4pt, rounded corners=2pt,   minimum width=30mm, minimum height=8mm,  align=center, fill=yellow!15, font=\scriptsize\itshape},
    final/.style  ={draw=green!55!black,  line width=0.7pt, rounded corners=2.5pt, minimum width=32mm, minimum height=10mm, align=center, fill=green!10},
    arr/.style    ={-{Latex[length=1.8mm,width=1.5mm]}, thick, draw=black!70},
    bg/.style     ={dashed, rounded corners=4pt, inner sep=2.8mm}
]

% ============ Stage 1: Marker palette ============
\node[inp]  (in1a)       at (0, 5.6) {brand colours};
\node[inp]  (in1b)       at (0, 4.4) {layout theme};
\node[inp]  (in1c)       at (0, 3.2) {products};

\node[proc] (palettesel) at (3.8, 4.4) {Marker palette\\selection};
\node[art]  (palette)    at (3.8, 2.0) {marker assignment\\$\textit{product} \rightarrow \textit{hex}$};
...
\begin{pgfonlayer}{background}
\node[bg, draw=gray!55, fill=gray!4,
      fit=(in1a)(in1c)(palettesel)(palette),
      label={[font=\scriptsize\bfseries\itshape, gray!75]above:Stage 1: marker palette}] {};
\end{pgfonlayer}

% ============ Stage 2: Outline rendering ============
\node[inp]    (in2a)   at (7.2, 5.0) {scene prompt};
\node[inp]    (in2b)   at (9.6, 5.0) {reference photos};

\node[aicall] (mdl)    at (8.4, 2.6) {Multimodal model\\\scriptsize(outline mode)\\[1pt]\tiny\textsc{ai call}};
\node[art]    (canvas) at (8.4, 0.4) {placeholder canvas\\\scriptsize(scene + outlined products)};
...
\begin{pgfonlayer}{background}
\node[bg, draw=blue!55, fill=blue!3,
      fit=(in2a)(in2b)(mdl)(canvas),
      label={[font=\scriptsize\bfseries\itshape, blue!65]above:Stage 2: outline rendering}] {};
\end{pgfonlayer}

% ============ Stage 3: Detect and composite ============
\node[proc]   (detect) at (13.6, 5.6) {Marker detect\\\scriptsize(Chebyshev match,\\\scriptsize largest component)};
\node[proc]   (mask)   at (13.6, 3.9) {Mask construction\\\scriptsize(flood-fill + dilate)};
\node[aicall] (lama)   at (13.6, 2.0) {LaMa inpainting\\\scriptsize(erase outlines,\\\scriptsize repaint background)\\[1pt]\tiny\textsc{ai call (IOPaint)}};
\node[proc]   (paste)  at (13.6, 0.0) {Scale \& alpha-composite\\\scriptsize(retailer photo paste)};
...
\node[final] (out) at (13.6, -1.7) {Final image\\\scriptsize(products faithful)};
\draw[arr] (paste.south) -- (out.north);

\begin{pgfonlayer}{background}
\node[bg, draw=orange!60, fill=orange!3,
      fit=(detect)(mask)(lama)(paste),
      label={[font=\scriptsize\bfseries\itshape, orange!70!black]above:Stage 3: detect and composite}] {};
\end{pgfonlayer}
...
\begin{enumerate}
\item[\textbf{(a)}] \emph{Per-product colour matching.} Each pixel of the placeholder canvas is mapped to the closest marker colour by Chebyshev RGB distance under a tolerance of $80$, producing a per-pixel product index (or $-1$ if no marker is within tolerance).
\item[\textbf{(b)}] \emph{Largest-component filter.} For each product, only the largest connected component of matched pixels is kept, discarding stray marker-coloured pixels in the background that would otherwise inflate the paste-back bounding box.
\item[\textbf{(c)}] \emph{Outline thickening.} The retained outline is dilated by four pixels to absorb anti-aliased halo pixels at the stroke edge.
\item[\textbf{(d)}] \emph{Inpaint mask construction.} A binary product-region mask is built by flood-filling unmatched cells from the canvas border and inverting the result, then dilated by fifteen pixels to also catch the model's occasional drop shadows.
\item[\textbf{(e)}] \emph{LaMa inpainting.} The placeholder canvas and the mask are passed to a LaMa inpainting model \cite{suvorov2022lama} (served via IOPaint), chosen because it handles large masks, runs on CPU at acceptable latency, and needs no inference-time text prompt.
\item[\textbf{(f)}] \emph{Product paste.} The retailer's source photograph is trimmed to its opaque bounding box, scaled by area to $0.8\times$ the median placeholder area, anchored at the placeholder's bottom-left corner so products ``stand'' on a baseline, and alpha-composited onto the inpainted canvas.
\end{enumerate}
...
\subsection{The quality gate: a language-model judge with retry}
...
If the per-trial acceptance probabilities are treated as approximately independent, capping the loop at three trials gives an effective acceptance rate of $1 - 0.30^{3} \approx 0.973$. Table~\ref{tab:hyb-acceptance} reports the corresponding rates for one, two and three trials. In practice, the user almost always receives an acceptable image after at most three internal retries, which matches the figures predicted by the independent-trial model and is consistent with the hands-on experience during development.
...
\hline
\end{tabularx}
\caption{Compound acceptance probability under the independent-trial model, given a single-trial acceptance rate of $0.70$.}
...
\begin{tikzpicture}[
    every node/.style={font=\scriptsize},
    inp/.style    ={draw=black!50,        line width=0.4pt, rounded corners=2pt,   minimum width=28mm, minimum height=11mm, align=center, fill=gray!6,   font=\scriptsize\itshape},
    proc/.style   ={draw=gray!65,         line width=0.5pt, rounded corners=2.5pt, minimum width=32mm, minimum height=12mm, align=center, fill=gray!12},
    aicall/.style ={draw=blue!60!black,   line width=0.7pt, rounded corners=2.5pt, minimum width=32mm, minimum height=14mm, align=center, fill=blue!8},
    final/.style  ={draw=green!55!black,  line width=0.7pt, cylinder, shape border rotate=90, aspect=0.25, minimum width=26mm, minimum height=12mm, align=center, fill=green!10},
    arr/.style    ={-{Latex[length=1.8mm,width=1.5mm]}, thick, draw=black!70},
    barr/.style   ={-{Latex[length=1.8mm,width=1.5mm]}, thick, dashed, draw=black!55}
]

\node[inp]    (src) at (0, 0)    {gallery image\\(native resolution)};
\node[proc]   (chk) at (4.6, 0)  {compute target pixels\\\scriptsize(paper size @ $300\,$dpi)};
\node[aicall] (up)  at (9.4, 1.9) {Real-ESRGAN\\tile upscaler\\[1pt]\tiny\textsc{ai call (ONNX)}};
\node[proc]   (lay) at (14.2, 0) {PDF layout\\\scriptsize($\pm$ QR code)};
\node[final]  (out) at (18.8, 0) {print-ready\\PDF in gallery};

\draw[arr]  (src) -- (chk);
\draw[arr]  (chk.north) to[out=60, in=180]  node[above, font=\tiny\itshape, pos=0.55] {if scale~$>1$} (up.west);
\draw[arr]  (up.east)   to[out=0,  in=120] (lay.north);
\draw[barr] (chk) -- node[above, font=\tiny\itshape] {else} (lay);
\draw[arr]  (lay) -- (out);
...
The pipeline is built on the same protocol-over-model-size principle as the previous two contributions. The model used is older and smaller than the multimodal models the rest of the platform relies on, but the protocol around it (fixed paper sizes, a hard $300\,$dpi target, deterministic tile padding) is what makes its output reliable enough to send to a printer.

\section{Closing remarks}
\label{sec:contributions-closing}

The three contributions described in this chapter share the same underlying argument. The recommendation engine grounds a language model in a curated playbook and a stream of external signals, and constrains it through a multi-stage chain whose intermediate artefacts are typed; the hybrid catalogue pipeline turns a multimodal model into an obedient scene generator by giving it a marker-outline contract, and uses a deterministic compositor to enforce that contract where the model breaks it; the print pipeline turns a smaller super-resolution model into a print-ready upscaler by tiling under fixed paper sizes and a hard dpi target. None of the three contributions is a new model. Each of them is a protocol that an existing model is asked to respect, together with the cheap deterministic machinery that handles the cases where it does not. The next chapter shows the assets the pipelines produce and describes the testing strategy that protects them.
