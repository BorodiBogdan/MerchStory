...
\definecolor{cTeal}{HTML}{20756C}   % stores
...
\node[card=cSlate, minimum width=30mm, minimum height=16mm] (screen) at (5.0, 0)
    {Asset-creation\\screen\\[2pt]prompt and category\\pre-filled};

\node[card=cIndigo, minimum width=30mm, minimum height=16mm] (pipe) at (10.0, 0)
    {Generation\\pipeline\\[2pt](catalogue or\\announcement)};

\node[card=cTeal, minimum width=26mm, minimum height=12mm] (out) at (14.4, 0)
...
\begin{tikzpicture}[
    ...
    svc/.style={draw=cSlate, line width=0.9pt, rounded corners=3pt, fill=cSlate!7,
                ...
                drop shadow={shadow xshift=0.5mm, shadow yshift=-0.5mm,
                             fill=black, opacity=0.12}},
    iface/.style={draw=cIndigo, line width=0.9pt, rounded corners=3pt,
                  fill=cIndigo!14, minimum width=50mm, minimum height=16mm,
                  align=center, text=cInk,
                  drop shadow={shadow xshift=0.5mm, shadow yshift=-0.5mm,
                               fill=black, opacity=0.12}},
...
\node[iface] (iface) at (5.6, 0) {\textbf{\texttt{IImageProvider}}\\[2pt]\texttt{GenerateAsync(}\\\texttt{prompt, inlineImages)}};

\node[prov, minimum width=40mm, minimum height=16mm] (prov) at (11.4, 0)
...
\begin{tikzpicture}[
    ...
    arrlbl/.style={font=\sffamily\tiny\itshape, fill=white, inner sep=1pt,
                   text=cInk!80},
    rowlbl/.style={font=\sffamily\small\bfseries, anchor=east, text=cInk!75}
]

% --- (a) pure generative ---
\node[card=cTeal]  (refA) at (0,    2.6) {Reference\\photographs};
\node[model]       (mdlA) at (4.4,  2.6) {Multimodal\\model};
\node[card=cAmber] (outA) at (9.4,  2.6) {Final image\\\textit{products distorted}};
\draw[arr] (refA) -- (mdlA);
\draw[arr] (mdlA) -- (outA);
...
\node[card=cSlate] (cmpB) at (9.4,  0) {Deterministic\\compositor};
...
\begin{tikzpicture}[
    font=\sffamily\scriptsize,
    inp/.style={draw=cTeal, line width=0.6pt, rounded corners=2.5pt, fill=white,
                minimum width=28mm, minimum height=11mm, align=center, text=cInk,
                font=\sffamily\scriptsize\itshape},
    ...
    aicall/.style={draw=cGray, line width=0.9pt, rounded corners=3pt, fill=cGray!6,
                   ...
                   minimum height=14mm, align=center, text=cInk},
...
\draw[arr]  (src) -- (chk);
\draw[arr]  (chk.north) to[out=60, in=180]  node[arrlbl, above, pos=0.55] {if scale~$>1$} (up.west);
\draw[arr]  (up.east)   to[out=0,  in=120] (lay.north);
\draw[barr] (chk) -- node[arrlbl, above] {else} (lay);
\draw[arr]  (lay) -- (out);
