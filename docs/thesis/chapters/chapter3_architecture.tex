...
\textbf{Daily marketing recommendations:} Once a day, the platform must produce a small ranked set of fresh marketing themes adapted to the retailer's shop and to the day's external context (weather, recent news, holidays). Each theme is presented as a fully drafted promotional idea, in the locked language, and the retailer may either dismiss it, react to it, or hand it directly to the asset-generation pipeline as a starting prompt. The recommendation engine that produces these themes is the first of the two main contributions of this thesis and is described in detail in Chapter 4.
...
\textbf{Administration:} A small set of capabilities is reserved to operators with the administrative flag set on their account. An operator can grant credits to a retailer's wallet and look up accounts by email, and can curate the shared reference catalogue by adding professional product images one at a time or importing a categorised archive of them in bulk. The administrative surface is deliberately spartan and lives inside the same client application as the retailer surface, with the privileged screens hidden from non-administrative accounts.
...
\hline
Administration          & Credit grants, account lookup and reference-catalogue curation, reserved to operators. \\
\hline
\end{tabularx}
\end{table}
...
Taken together, these managed services let the platform satisfy its non-functional requirements by construction: Container Apps' autoscaling and the stateless server hold interactive reads and writes within their latency target, the asynchronous job model keeps long-running AI calls off the request path, and the platform ingress, the per-tenant rate limiter and Key Vault-backed secrets cover availability, cost control and security without any bespoke infrastructure. The Application Insights traces and Log Analytics dashboards described above are what confirm these bounds hold in production.