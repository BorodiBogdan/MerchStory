...
\hline
\endlastfoot
\texttt{AppUser} & Identifies a retailer (or operator) and tracks the credit balance. \\
\hline
\texttt{ShopProfile} & Holds the brand identity and contact details captured during onboarding. \\
\hline
\texttt{Product} & An item the retailer sells, with name, price and a representative photograph. \\
\hline
\texttt{GeneratedImage} & An entry in the gallery, tagged with its asset family. \\
\hline
\texttt{CoinTransaction} & A row of the credit ledger that records a debit or refund and, optionally, the asset it paid for. \\
\hline
\texttt{DailyRecommendation} & The day's marketing ideas together with the context snapshot used to produce them. \\
\hline
\texttt{IdeaEmbedding} & A per-idea text embedding used to deduplicate themes against the shop's recent ideas. \\
\hline\hline
\texttt{Category} & A node in the reference catalogue's hierarchical tree. \\
\hline
\texttt{ReferenceImage} & A curated product photograph with a CLIP embedding for search-by-photo. \\
\hline
\texttt{PromoPlaybookEntry} & A known-good promotional pattern with a text embedding the recommendation engine retrieves over. \\
\hline
\texttt{Holiday} & A cached public holiday for a country and year, used as a context signal. \\
\end{tabularx}
}
