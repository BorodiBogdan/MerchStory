...
\definecolor{cTeal}{HTML}{20756C}   % data plane
...
% --- same palette as Figure \ref{fig:three-tier} ---
\definecolor{cInk}{HTML}{2E3440}    % text + arrows
\definecolor{cIndigo}{HTML}{3F4D8C} % Semantic Kernel / Azure
\definecolor{cSlate}{HTML}{4A5568}  % application server
...
\begin{tikzpicture}[
    ...
    groupcard/.style={draw=#1, line width=0.9pt, rounded corners=3pt, inner sep=0pt,
                      ...
                      minimum width=62mm, minimum height=17mm},
    tab/.style={fill=#1, rounded corners=2.5pt, inner xsep=6pt, inner ysep=0pt,
                minimum height=5.2mm, anchor=south west,
                font=\sffamily\scriptsize\bfseries, text=white},
    chip/.style={draw=#1, line width=0.6pt, rounded corners=2.5pt, fill=white,
                 minimum width=27mm, minimum height=11mm, align=center, text=cInk},
    arr/.style={-{Latex[length=2mm,width=1.5mm]}, line width=0.7pt, draw=cInk!85}
]

% ============ layer bars ============
\node[bar=cSlate, fill=cSlate!8, minimum width=146mm, minimum height=10mm]
      (api) at (0,4.6) {\textbf{Application server} \quad (ASP.NET Core, .NET 10)};
\node[bar=cIndigo, fill=cIndigo!14, minimum width=146mm, minimum height=13mm]
      (sk) at (0,2.85)
      {\textbf{Semantic Kernel}\\prompt templates \;$\cdot$\; retrieval-augmented context \;$\cdot$\; multi-stage chaining};

% ============ provider group cards ============
\node[groupcard=cIndigo] (azureCard) at (-4.0,0.3) {};
\node[groupcard=cGray]   (extCard)   at ( 4.0,0.3) {};
% ============ provider chips ============
\node[chip=cIndigo] (aoai) at (-5.45,0.3) {Azure OpenAI};
\node[chip=cIndigo] (afnd) at (-2.55,0.3) {Azure AI\\Foundry};
\node[chip=cGray]   (gem)  at ( 2.55,0.3) {Google Gemini};
\node[chip=cGray]   (nb)   at ( 5.45,0.3) {Nano Banana};
...
\node[tab=cIndigo] at ([xshift=4mm]azureCard.north west) {Azure ecosystem};
\node[tab=cGray]   at ([xshift=4mm]extCard.north west)   {External providers};
...
\begin{tikzpicture}[
    font=\sffamily\scriptsize,
    bar/.style={draw=#1, line width=0.9pt, rounded corners=3pt, align=center,
                minimum width=78mm, minimum height=9mm, text=cInk,
                drop shadow={shadow xshift=0.5mm, shadow yshift=-0.5mm,
                             fill=black, opacity=0.12}},
...
\node[bar=cIndigo, fill=cIndigo!7]  (ep)   at (0,0)    {Endpoints (\texttt{Map\dots Endpoints})};
\node[bar=cSlate,  fill=cSlate!8]   (svc)  at (0,-1.7) {Feature services};
\node[bar=cIndigo, fill=cIndigo!14] (orch) at (0,-3.4) {Semantic Kernel + \texttt{IImageProvider}};
\node[extbar]                       (http) at (0,-5.1) {External HTTP boundary};
\draw[arr] (ep)   -- (svc);
\draw[arr] (svc)  -- (orch);
\draw[arr] (orch) -- (http);
\end{tikzpicture}
...
\definecolor{cTeal}{HTML}{20756C}   % vector-bearing entities + data plane
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
    font=\sffamily\scriptsize,
...
\definecolor{cIndigo}{HTML}{3F4D8C} % client
\definecolor{cSlate}{HTML}{4A5568}  % application server
\definecolor{cTeal}{HTML}{20756C}   % data stores
...
\definecolor{cTeal}{HTML}{20756C}   % data + secrets
...
\draw[arr] (acr) -- node[above]{pull image} (web);
% api -> data plane
