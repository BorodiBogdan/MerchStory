...
The web build of the client runs as its own Azure Container App (\texttt{merchstory-web}), pulling its image from the same registry as the API. The mobile builds for iOS and Android are distributed through their respective application stores and are not deployed to Azure.
...
The VNet topology is conservative. The database, the blob storage account and Key Vault are reachable only through private endpoints inside the VNet, with their public network access disabled at the resource level. The Container Apps Environment sits in the same VNet and exposes only its built-in HTTPS ingress to the Internet.
