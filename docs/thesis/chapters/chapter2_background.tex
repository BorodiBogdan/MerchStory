...
\definecolor{cIndigo}{HTML}{3F4D8C} % learned denoiser (model)
\definecolor{cGray}{HTML}{7C8390}   % image / noise states
\begin{tikzpicture}[
    node distance=10mm and 13mm,
    font=\sffamily\scriptsize,
    every node/.style={text=cInk},
    box/.style={draw=cGray, line width=0.6pt, rounded corners=2.5pt, fill=white,
...
\node[right=5mm of x1] (dots) {$\cdots$};
\node[box, right=5mm of dots] (xt1) {$x_{T-1}$};
\node[box, right=of xt1] (xt) {$x_T$};

\draw[arr] (x0) -- node[above=0.5mm] {$+\,\text{noise}$} (x1);
\draw[arr] (x1) -- (dots);
\draw[arr] (dots) -- (xt1);
\draw[arr] (xt1) -- node[above=0.5mm] {$+\,\text{noise}$} (xt);

\draw[arr, draw=cIndigo, dash pattern=on 2.4pt off 1.7pt]
    (xt.south) to[bend left=32]
    node[below=1.2mm, text=cIndigo] {denoiser $\epsilon_\theta$, conditioned on text $c$} (x0.south);
\end{tikzpicture}
...
\centering
% --- self-contained palette (muted, print-friendly) ---
\definecolor{cInk}{HTML}{2E3440}    % text
\definecolor{cIndigo}{HTML}{3F4D8C} % prompt sections
\begin{tikzpicture}[
    font=\sffamily\scriptsize,
    block/.style={draw=cIndigo, line width=0.6pt, rounded corners=2.5pt, fill=white,
                  text width=86mm, align=left, inner xsep=8pt,
                  minimum height=8mm, anchor=west, text=cInk}
]
\node[block] (role) at (0,0)         {\textcolor{cIndigo}{\textbf{Role.}}\;\; ``You are an art director at a top creative studio.''};
\node[block, below=2.2mm of role.south west, anchor=north west] (ctx) {\textcolor{cIndigo}{\textbf{Brand context.}}\;\; name, slogan, colours, business domain.};
\node[block, below=2.2mm of ctx.south west, anchor=north west] (layout) {\textcolor{cIndigo}{\textbf{Layout zones.}}\;\; header (15\%), hero (70\%), footer (15\%).};
\node[block, below=2.2mm of layout.south west, anchor=north west] (rules) {\textcolor{cIndigo}{\textbf{Content rules.}}\;\; what to include, tone, currency formatting.};
\node[block, below=2.2mm of rules.south west, anchor=north west] (neg) {\textcolor{cIndigo}{\textbf{Negative constraints.}}\;\; no sparkles, no fake bokeh, no clipart.};
\end{tikzpicture}
...
% --- self-contained palette (muted, print-friendly) ---
\definecolor{cInk}{HTML}{2E3440}    % text + arrows
\definecolor{cIndigo}{HTML}{3F4D8C} % language model / generation
\definecolor{cTeal}{HTML}{20756C}   % retrieval / vector store
\definecolor{cAmber}{HTML}{B07A1F}  % augmentation accent
\definecolor{cGray}{HTML}{7C8390}   % raw query
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
    font=\sffamily\scriptsize,
    box/.style={draw=#1, line width=0.6pt, rounded corners=2.5pt, fill=white,
                minimum width=18mm, minimum height=10mm, align=center,
                inner sep=2pt, text=cInk},
    store/.style={draw=cTeal, line width=0.6pt, cylinder, shape border rotate=90,
                  aspect=0.25, minimum width=18mm, minimum height=13mm, align=center,
                  cylinder uses custom fill, cylinder body fill=white,
                  cylinder end fill=cTeal!15, inner sep=1pt, text=cInk},
    phaseband/.style={draw=#1, line width=0.9pt, rounded corners=3pt, fill=#1!7,
...
\node[box=cGray] (q) {Query};
\node[box=cIndigo, right=4mm of q] (emb) {Embedding\\encoder};
\node[store, right=4mm of emb] (idx) {Vector\\index};
\node[box=cTeal, right=4mm of idx] (top) {Top-\(k\)\\retrieved};
\node[box=cAmber, right=8mm of top] (prompt) {Augmented\\prompt};
\node[box=cIndigo, right=8mm of prompt] (llm) {Language\\model};
\node[box=cIndigo, right=4mm of llm] (ans) {Grounded\\answer};

\begin{scope}[on background layer]
    \node[phaseband=cTeal,   fit=(q)(top)(idx)] (rband) {};
    \node[phaseband=cAmber,  fit=(prompt)(prompt.center |- idx.north)(prompt.center |- idx.south)] (aband) {};
    \node[phaseband=cIndigo, fit=(llm)(ans)(llm.center |- idx.north)(llm.center |- idx.south)] (gband) {};
\end{scope}
...
% phase tabs (drawn last so they sit above the band borders)
\node[tab=cTeal]   at ([xshift=4mm]rband.north west) {RETRIEVE};
\node[tab=cAmber]  at ([xshift=4mm]aband.north west) {AUGMENT};
\node[tab=cIndigo] at ([xshift=4mm]gband.north west) {GENERATE};
\end{tikzpicture}}
...
% --- self-contained palette (muted, print-friendly) ---
\definecolor{cInk}{HTML}{2E3440}    % text + arrows
\definecolor{cIndigo}{HTML}{3F4D8C} % encoders + image embeddings
\definecolor{cTeal}{HTML}{20756C}   % embedding space + text embeddings
\definecolor{cAmber}{HTML}{B07A1F}  % highlighted pair
\definecolor{cGray}{HTML}{7C8390}   % raw inputs

\begin{tikzpicture}[
    font=\sffamily\scriptsize,
    every node/.style={text=cInk},
    inp/.style={draw=cGray, line width=0.6pt, dash pattern=on 2.4pt off 1.7pt,
                rounded corners=2.5pt, fill=white, minimum width=24mm,
                minimum height=10mm, align=center},
    enc/.style={draw=cIndigo, line width=0.9pt, rounded corners=3pt, fill=cIndigo!14,
                minimum width=24mm, minimum height=11mm, align=center,
                drop shadow={shadow xshift=0.5mm, shadow yshift=-0.5mm,
                             fill=black, opacity=0.12}},
    arr/.style={-{Latex[length=2mm,width=1.5mm]}, line width=0.7pt, draw=cInk!85},
    pairLine/.style={dotted, line width=0.8pt, draw=cInk!55},
    imgDot/.style={circle, fill=cIndigo, minimum size=2.4mm, inner sep=0pt},
    txtDot/.style={circle, fill=cTeal, minimum size=2.4mm, inner sep=0pt}
]

% Inputs
\node[inp] (img) at (0, 1.4) {Image};
\node[inp] (txt) at (0,-1.4) {\itshape ``a coffee mug''};

% Encoders
\node[enc] (imgEnc) at (3.6, 1.4) {Image\\encoder};
\node[enc] (txtEnc) at (3.6,-1.4) {Text\\encoder};

% Shared embedding space (ellipse)
\draw[line width=0.9pt, draw=cTeal, fill=cTeal!6,
      drop shadow={shadow xshift=0.5mm, shadow yshift=-0.5mm,
                   fill=black, opacity=0.12}]
      (8.2, 0) ellipse (2.7cm and 2.05cm);
\node[font=\sffamily\scriptsize\itshape, text=cTeal, anchor=south] at (8.2, 2.15) {shared embedding space};

% Background matched pairs (image dot near text dot)
\node[imgDot] (i1) at (6.9, 1.35) {};
\node[txtDot] (t1) at (7.3, 1.05) {};
\draw[pairLine] (i1) -- (t1);

\node[imgDot] (i2) at (9.15, 0.7) {};
\node[txtDot] (t2) at (9.55, 1.05) {};
\draw[pairLine] (i2) -- (t2);

\node[imgDot] (i3) at (7.6,-1.1) {};
\node[txtDot] (t3) at (7.2,-1.45) {};
\draw[pairLine] (i3) -- (t3);

\node[imgDot] (i4) at (9.2,-0.6) {};
\node[txtDot] (t4) at (9.55,-0.95) {};
\draw[pairLine] (i4) -- (t4);

% Highlighted pair (the example coffee-mug pair)
\node[imgDot, minimum size=3.4mm] (iHi) at (8.2, 0.1) {};
\node[txtDot, minimum size=3.4mm] (tHi) at (8.55,-0.12) {};
\draw[line width=0.6pt, draw=cInk!60] (iHi) -- (tHi);
\draw[dash pattern=on 2.4pt off 1.7pt, line width=0.8pt, draw=cAmber] (8.375,-0.01) circle (5.5mm);

% Arrows: inputs to encoders, encoders to highlighted pair
\draw[arr] (img) -- (imgEnc);
\draw[arr] (txt) -- (txtEnc);
\draw[arr] (imgEnc.east) to[out=0, in=160] (iHi);
\draw[arr] (txtEnc.east) to[out=0, in=200] (tHi);

% Legend
\node[anchor=west] at (5.7,-2.55) {\textcolor{cIndigo}{$\bullet$}\;image embeddings};
\node[anchor=west] at (8.5,-2.55) {\textcolor{cTeal}{$\bullet$}\;text embeddings};
