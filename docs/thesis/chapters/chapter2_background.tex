...
Three basic prompting styles frame the rest of this section: \emph{zero-shot} asks the model to perform a task without examples; \emph{few-shot} supplies a handful of input--output examples for the model to generalise from; and \emph{chain-of-thought} \cite{wei2022cot} invites the model to verbalise its intermediate reasoning. The proposed application uses zero-shot for image generation, where in-context examples would themselves have to be images and would inflate the token budget, and chain-of-thought in the recommendation engine of Section~\ref{sec:rag}, where a sequence of language-model calls reasons about which marketing themes to propose before drafting them. Beyond these basics, two further techniques recur throughout the platform: layout-aware prompting and multimodal prompting.
