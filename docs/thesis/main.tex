...
\usepackage{graphicx}
\usepackage{tikz}
\usetikzlibrary{positioning,arrows.meta,fit,shapes.geometric,shapes.multipart,backgrounds,shadows}
\usepackage{pgfplots}
...
\title{Automating Marketing Assets through Generative AI and Retrieval-Augmented Recommendation}
...
Chapter 1 motivates the work and states its objectives. Chapter 2 introduces the theoretical foundations: image-generating models, prompt design, retrieval-augmented generation and learned visual similarity. Chapter 3 describes the system architecture and its deployment on Microsoft Azure. Chapter 4 presents three technical contributions: a recommendation engine that pairs retrieval-augmented context with a fine-tuned writer model to produce fresh marketing themes from a curated promotional playbook, a hybrid generation technique that preserves the retailer's product photograph while the model designs the surrounding scene, and a print pipeline that upscales finished assets with a learned super-resolution model so they remain sharp from A6 to A3. Chapter 5 reports the platform's results, sample generated assets, the layered automated test suite and an offline evaluation harness that compares candidate writer models on a frozen reference set.
