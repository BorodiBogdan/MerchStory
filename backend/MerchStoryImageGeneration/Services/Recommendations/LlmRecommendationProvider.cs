...
public class LlmRecommendationProvider : IRecommendationProvider
{
    ...
    private static string BuildStrategistPrompt(RecommendationContext ctx)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("== TASK ==");
        sb.AppendLine($"Pick exactly {ctx.IdeasPerDay} DISTINCT angles, each on a DIFFERENT topic. Not variations of one idea.");
        sb.AppendLine("Diversity is required. Follow these rules:");
        sb.AppendLine("  - AT MOST ONE angle may be about the weather (rain, storm, heat, cold). Never make two weather angles, even if rain is the strongest signal.");
        sb.AppendLine("  - Every angle must use a DIFFERENT triggerSignal. Do not point two angles at the same signal.");
        sb.AppendLine("  - Vary the tone where the signals allow it. Do not give every angle the same tone.");
        sb.AppendLine("If the live signals above are too thin to fill all the slots with genuinely different topics (for example the");
        sb.AppendLine("only real signal is rain), that is normal. Fill the remaining slots with generic angles built from TODAY'S DATE");
        sb.AppendLine("and what this shop sells: pick different product categories the shop stocks, lean on the current season, and mix");
        sb.AppendLine("promotions with announcements. One weather angle plus several solid seasonal angles is the goal, not four rain angles.");
        sb.AppendLine();
        sb.AppendLine("GOOD set (4 different angles, only one about weather):");
        sb.AppendLine("  'Rainy weekend soup ingredients' (weather), 'Local strawberries are in season' (trend),");
        sb.AppendLine("  'Buy 3 bags of chips, get 1 free' (trend), 'Open late Friday for the match' (news).");
        sb.AppendLine("BAD set (4 weather angles, all the same topic, never do this):");
        sb.AppendLine("  'Rainy weekend comfort food', 'Stay-in snacks for the rainy weekend',");
        sb.AppendLine("  'Rain Saturday: hot drinks', 'Wet weekend pantry stockup'.");
