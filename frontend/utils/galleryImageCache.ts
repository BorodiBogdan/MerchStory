...
import { fetchGalleryImage } from '@/utils/api';
...
export function useGalleryImage(id: string): HookState {
  const [state, setState] = useState<HookState>(() => snapshotFor(id));

  useEffect(() => {
    // Re-sync on id change in case this hook is reused for a different id.
    setState(snapshotFor(id));

    // Kick off fetch if the image isn't cached yet.
    ...
    // Listen for external mutations (prime/evict from other components).
